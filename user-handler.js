...
export async function respondChannelData(newChannelID, oldChannelID) {
    let channel = await db.getChannelShallow({ '_id': newChannelID });
...
export async function respondMessageBatch(offset, channelID) {
    let channel = await db.getChannelDeep({ _id: channelID });
    server.to(socket.id).emit('Respond message batch', db.getMessageBatch(channel, offset, 50));
}
