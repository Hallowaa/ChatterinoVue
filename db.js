 ...
 * @param {number} offset Basically the amount of messages the user has already loaded
 * @param {number} amount The amount of messages to return
 */
export function getMessageBatch(channel, offset, amount) {
    let messages = channel.content.messages;
    let result = [];

    const from = Math.max(0, messages.length - offset - amount);
    const to = Math.max(0, messages.length - offset);
